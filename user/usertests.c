...
void klttest()
{
  uint64 stack_a = (uint64)malloc(kSTACK_SIZE);
  uint64 stack_b = (uint64)malloc(kSTACK_SIZE);

  int kt_a = kthread_create((void *(*)())kthread_start_func,  (void*)stack_a, kSTACK_SIZE);
  if(kt_a <= 0){
    printf("kthread_create failed\n");
    exit(1);
  }
  int kt_b = kthread_create((void *(*)())kthread_start_func,  (void*)stack_b, kSTACK_SIZE);
  if(kt_a <= 0){
    printf("kthread_create failed\n");
    exit(1);
  }
